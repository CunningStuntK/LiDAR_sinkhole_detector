# =====================================================================================================================
# Filename:     LiDAR_Basic_Detector_1_0.py
# Written by:   Keith Cusson                Date: Apr 2023
...
class Project(object):
    ...
    def __init__(self):
        ...
        self.DEMTiles = RasterCollection()
        self.DEM = Raster()
        self.waterFeatures = ShapeFile()
        self.AOI = ShapeFile()
        self.depressions = ShapeFile()
        self.sinkholes = ShapeFile()
    ...
    def __call__(self, name, path):
        ...
        for folder in folders:
            ...
            # Create a folder for tiles in appropriate folders
            if folder not in ['json', 'metadata', 'shapefiles']:
                sub_folder += f'/tiles'
                if not os.path.exists(sub_folder):
                    os.makedirs(sub_folder)
    ...
    def load(self, saveFile):
        ...
        try:
            ...
            self.DEMTiles.load(data['dem_tiles'])
            self.DEM.load(data['DEM'])
            self.waterFeatures.load(data['water'])
            self.AOI.load(data['AOI'])
            self.depressions.load(data['depressions'])
            self.sinkholes.load(data['sinkholes'])
    ...
    def save(self):
        saveDict = {'name': self.name,
                    ...
                    'dem_tiles': self.DEMTiles.export(),
                    'DEM': self.DEM.save(),
                    'water': self.waterFeatures.save(),
                    'AOI': self.AOI.save(),
                    'depressions': self.depressions.save(),
                    'sinkholes': self.sinkholes.save()}
...
class Raster(object):
    ...
    def __call__(self, path, name):
        ...
        rast = gdal.Open(self.path)
        band = rast.GetRasterBand(1)
        self.range = band.ComputeRasterMinMax()

        # Set objects to NULL to close the file
        band = None
        rast = None
...
class PointCloud(object):
    ...
    def createDEM(self, outPath, name) -> Raster:
...
class DetectorGUI(object):
    ...
    def gen_DEM(self):
        ...
        if len(self.proj.mergedFiles.clouds) == 1:
            self.print('Generating DEM. Please wait...')
            self.proj.DEM = self.proj.mergedFiles(-1).createDEM('rasters',
                                                                f'{self.proj.fileName[:-5]}')
            self.print(f'DEM generated in {secFmt(self.proj.DEM.time)}')
        ...
        # Otherwise, each tile will be converted to a raster, and then they will be merged.
            ...
            for i in range(0, len(self.proj.mergedFiles.clouds)):
                self.print(f'Generating DEM tile for {self.proj.mergedFiles(i).name}')
                self.proj.DEMTiles.add(self.proj.mergedFiles(i).createDEM('rasters/tiles',
                                                                          self.proj.mergedFiles(i).name))
                self.print(f'{self.proj.mergedFiles(i).name} DEM tile generated in '
                           f'{secFmt(self.proj.DEMTiles(-1).time)}\n')
            self.print(f'Merging {len(self.proj.DEMTiles.tifs)} DEM tiles. Please wait...')
            self.proj.DEM = self.proj.DEMTiles.merge(f'rasters/{self.proj.fileName[:-4]}tif',
                                                     self.proj.fileName[:-5])
            self.print(f'DEM merged in {secFmt(self.proj.DEM.time)}')

        # Fill any voids in the DEM
        self.print('Filling DEM voids.')
        self.proj.DEM.fillVoids()
        self.print('DEM voids filled.')

        # Add the DEM to the QGIS project, and move it to the bottom
        # Taken from the PyQGIS Cookbook
        rastLayer = self.proj.qgsProj.addMapLayer(self.proj.DEM.qObj, False)  # Don't display map layer
        root = self.proj.qgsProj.layerTreeRoot()                              # Find the layer tree root
        root.insertLayer(-1, rastLayer)                                       # Insert the raster at the end
    ...
    def findSink(self):
        self.win['_OUT_PRINT_' + Gui.WRITE_ONLY_KEY].print('=' * 60)
        self.print('Finding sinkholes. Please wait')
        ...
        startTime = time.time()
        arg_params = {'input': self.proj.DEM.qObj,
                      'format': 1,
                      'output': f'rasters/{self.proj.fileName[:-5]}_filled.tif',
                      'direction': f'rasters/{self.proj.fileName[:-5]}_dir.tif',
                      'areas': f'rasters/{self.proj.fileName[:-5]}_areas.tif'}

        processing.run('grass7:r.fill.dir', arg_params)

        filled_DEM = Raster()
        filled_DEM(f'rasters/{self.proj.fileName[:-5]}_filled.tif', 'Filled DEM')
        filled_DEM.time = time.time() - startTime
        self.print(f'Filled DEM generated in {secFmt(filled_DEM.time)}\n')

        # The raster calculator is used to compute the difference between the filled DEM and the original DEM to create
        # a preliminary sinkhole raster
        self.print('Creating preliminary sinkhole raster.')
        startTime = time.time()
        arg_params = {'INPUT_A': filled_DEM.qObj,
                      'BAND_A': 1,
                      'INPUT_B': self.proj.DEM.qObj,
                      'BAND_B': 1,
                      'FORMULA': 'A - B',
                      'RTYPE': 5,
                      'OUTPUT': f'rasters/{self.proj.fileName[:-5]}_diff.tif'}

        processing.run('gdal:rastercalculator', arg_params)

        difference_raster = Raster()
        difference_raster(f'rasters/{self.proj.fileName[:-5]}_diff.tif', 'Difference Raster')
        difference_raster.time = time.time() - startTime
        self.print(f'Preliminary sinkhole raster generated in {secFmt(difference_raster.time)}\n')
        ...
        if not os.path.exists(f'shapefiles/AOI_edited/'):
            os.makedirs(f'shapefiles/AOI_edited')

        startTime = time.time()
        arg_params = {'INPUT': self.proj.qgsProj.mapLayersByName('AOI')[0],
                      'OVERLAY': temp_poly,
                      'OUTPUT': f'shapefiles/AOI_edited/AOI_edited.shp'}

        processing.run('qgis:difference', arg_params)
        clip_AOI = ShapeFile()
        clip_AOI(f'shapefiles/AOI_edited/AOI_edited.shp', 'Edited AOI')
        ...
        startTime = time.time()
        arg_params = {'INPUT': difference_raster.qObj,
                      'MASK': clip_AOI.qObj,
                      'SOURCE_CRS': difference_raster.qObj.crs(),
                      'TARGET_CRS': difference_raster.qObj.crs(),
                      'TARGET_EXTENT': difference_raster.qObj,
                      'OUTPUT': f'rasters/{self.proj.fileName[:-5]}_diff_clipped.tif'}

        processing.run('gdal:cliprasterbymasklayer', arg_params)

        clipped_difference = Raster()
        clipped_difference(f'rasters/{self.proj.fileName[:-5]}_diff_clipped.tif',
                           'Clipped Difference Raster')
        ...
        startTime = time.time()
        arg_params = {'input': clipped_difference.qObj,
                      'method': 0,
                      'size': 3,
                      'output': f'rasters/{self.proj.fileName[:-5]}_filt.tif'}

        processing.run('grass7:r.neighbors', arg_params)

        filt_rast = Raster()
        filt_rast(f'rasters/{self.proj.fileName[:-5]}_filt.tif',
                  'Filtered Raster')
        self.print(f'Sinkhole raster smoothed in {secFmt(time.time() - startTime)}\n')

        # The resulting raster is reclassified to create a binary raster. A value of 1 will be assigned to cells that
        # are deeper than 0.15m which corresponds to the vertical accuracy of LiDAR data. All other cells will have a
        # value of 0.
        self.print('Classifying sinkholes by depth.')
        # Calculate the maximum value of the raster
        r_max = filt_rast.range[1]

        arg_params = {'INPUT_RASTER': filt_rast.qObj,
                      'RASTER_BAND': 1,
                      'TABLE': ['0.15', f'{numpy.ceil(r_max) + 1}', '1'],
                      'OUTPUT': f'rasters/{self.proj.fileName[:-5]}_reclass.tif'}

        processing.run('native:reclassifybytable', arg_params)

        class_rast = Raster()
        class_rast(f'rasters/{self.proj.fileName[:-5]}_reclass.tif', 'Reclassified Raster')
        ...
        # .............................................................................................................
        # Each processing algorithm generates a new shapefile, and joining them will produce all the fields required to
        # categorize each feature to determine if they are likely to be a sinkhole. Rather than saving multiple
        # shapefiles to the project folder, temporary outputs are used to generate each of the fields required, and a
        # new shapefile will be created when all the fields have been joined.
        # .............................................................................................................
        ...
        # Add a feature id to each feature in the vector layer so multiple files can be joined later in the algorithm
        ...
        # For the remaining attributes, they can be computed with successive instances of the field calculator algorithm
        ...
        # Once all fields have been calculated, save the layer, and add it to the project
        self.print('Saving shapefile.')
        poly_root = f'shapefiles/{self.proj.fileName[:-5]}_sink_rast_smoothed'
        ...
        self.proj.depressions(f'{poly_root}/{poly_fn}', 'Smoothed Depressions')
        self.proj.depressions.createQgsLayer()

        self.proj.qgsProj.addMapLayer(self.proj.depressions.qObj)

        # # Apply a style to the depressions and reduce the layer opacity to diminish the importance of these polygons
        layer = self.proj.qgsProj.mapLayersByName('Smoothed Depressions')[0]
        style = QgsStyle.defaultStyle().symbol('simple orange fill')
        layer.renderer().setSymbol(style)
        layer.triggerRepaint()
        self.proj.qgsProj.mapLayersByName('Smoothed Depressions')[0].setOpacity(0.4)

        # Create a shapefile that has only those depressions with a "Sinkhole Score" of greater than 0.5

        self.proj.depressions.filteredCopy(f'{self.proj.fileName[:-5]}_sinkholes',
                                           'Potential Sinkholes',
                                           'Sinkhole_S >= 0.5')
        sinkPath = f'shapefiles/{self.proj.fileName[:-5]}_sinkholes/{self.proj.fileName[:-5]}_sinkholes.shp'
        self.proj.sinkholes(sinkPath, 'Potential Sinkholes')

        # Add the shapefile to the project
        self.proj.sinkholes.createQgsLayer()
        self.proj.qgsProj.addMapLayer(self.proj.sinkholes.qObj)

        # Apply a style to the sinkholes
        layer = self.proj.qgsProj.mapLayersByName('Potential Sinkholes')[0]
        style = QgsStyle.defaultStyle().symbol('simple green fill')
        layer.renderer().setSymbol(style)
        layer.triggerRepaint()
    ...
    def results(self):
        ...
        table_vals = []
        layer = self.proj.sinkholes.oObj.GetLayer()

        for i in range(5):
            filt = f"Sinkhole_S <= {1.0 - i * 0.1} AND Sinkhole_S > {0.9 - i * 0.1}"
            layer.SetAttributeFilter(filt)
            table_vals.append([f'{90 - i * 10} - {100 - i * 10}%', layer.GetFeatureCount()])

        # Create a layout for the results window.
        layout = [[Gui.Text('Results',
                            font=('Arial', 13, 'bold'))],
                  [Gui.Text(f'Potential sinkholes found: {self.proj.sinkholes.count}\n'
                            f'Total depressions found: {self.proj.depressions.count}')],
                  [Gui.Text('Sinkhole breakdown')],
                  [Gui.Table(values=table_vals,
